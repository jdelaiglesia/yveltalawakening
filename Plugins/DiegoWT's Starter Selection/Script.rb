#===============================================================================
# DiegoWT's Starter Selection script (modified by Dafting/Dragonite)
#
# NOTE: This script is intended to be used ONLY on Pokémon: Yveltal's Awakening.
#       It will not work on any other game.
#
#       If you want the original script, go to the following link:
#       https://reliccastle.com/resources/747/
#===============================================================================
class DiegoWTsStarterSelection
  def initialize(pkmn1,pkmn2,pkmn3)
    ...
    @sprites["fakemon_1"] = IconSprite.new(128,10,@viewport)
    @sprites["fakemon_1"].opacity = 0
    @sprites["fakemon_1"].z = 99999

    @sprites["fakemon_2"] = IconSprite.new(128,20,@viewport)
    @sprites["fakemon_2"].opacity = 0
    @sprites["fakemon_2"].z = 99999

    @sprites["fakemon_3"] = IconSprite.new(148,20,@viewport)
    @sprites["fakemon_3"].opacity = 0
    @sprites["fakemon_3"].z = 99999

    if $StarterShinyEvent == "1"
      echo "Shiny starter"
      @sprites["fakemon_1"].setBitmap("Graphics/Pictures/DiegoWT's Starter Selection/ShinyRenadett")
      @sprites["fakemon_2"].setBitmap("Graphics/Pictures/DiegoWT's Starter Selection/ShinyBrantra")
      @sprites["fakemon_3"].setBitmap("Graphics/Pictures/DiegoWT's Starter Selection/ShinySnokant")
    else
      echo "Normal starter"
      @sprites["fakemon_1"].setBitmap("Graphics/Pictures/DiegoWT's Starter Selection/Renadett")
      @sprites["fakemon_2"].setBitmap("Graphics/Pictures/DiegoWT's Starter Selection/Brantra")
      @sprites["fakemon_3"].setBitmap("Graphics/Pictures/DiegoWT's Starter Selection/Snokant")
    end
  ...
  def pbOpenScene
    ...
    @sprites["textbox"].text = _INTL("<ac>Anda, elige el que quieras.</ac>")
    pbStartChoosing
  end
  ...
  def pbChooseBall
    ...
    if @pokemon.type2 != @pokemon.type1 && StarterSelSettings::TYPE2COLOR
      ...
      @sprites["textbox"].text = _INTL("<ac>Entonces, ¿te quedarás con #{@pkmnname}, <br>el Pokémon tipo #{@type}?</ac>")
    end
    ...
    10.times do
      pbUpdateSpriteHash(@sprites)
      @sprites["fakemon_#{@select}"].opacity += 255/10
      pbWait(1)
    end
    ...
    if confirm == 1
      ...
      pbCloseScene
      if $StarterShinyEvent != "1"
        pbAddPokemon(@data["pkmn_#{@select}"],StarterSelSettings::STARTERL,true,false)
      elsif $StarterShinyEvent == "1"
        pbAddPokemon(@data["pkmn_#{@select}"],StarterSelSettings::STARTERL,true,true)
      else
        pbAddPokemon(@data["pkmn_#{@select}"],StarterSelSettings::STARTERL,true,false)
      end
    else
      @sprites["textbox"].y = @oldMsgY
      @sprites["textbox"].text = _INTL("<ac>Elige el que quieras.</ac>")
      10.times do
        pbUpdateSpriteHash(@sprites)
        @sprites["fakemon_#{@select}"].opacity -= 255/10
        pbWait(1)
